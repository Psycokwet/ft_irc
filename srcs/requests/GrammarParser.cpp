...
GrammarParser::GrammarParser(const GrammarParser &src) : _vars(src._vars),
														 ...
														 _parsedRequest(src._parsedRequest)
{
	(void)src;
}
...
bool GrammarParser::saveIfNecesary()
{
	...
	{
		this->_parsedRequest->add_value_parsedDatas(_priority_states.front()->getBuffer());
		_saveType = NO_VAR_TYPE;
		return true;
	}
	else if (frontType == KEY)
	{
		this->_parsedRequest->add_key_parsedDatas(_priority_states.front()->getBuffer());
		_saveType = NO_VAR_TYPE;
		return true;
	}
	return false;
...
void GrammarParser::initParse()
{
	if (_priority_states.size() != 0)
		return;
	this->_parsed_datas.clear();
	this->_parsedRequest = new ParsedRequest();
...
{
	initParse();
	if (_parsedRequest->get_parsing_validity_state() >= PARSE_FAILURE)
		return _parsedRequest->get_parsing_validity_state();
	do
	{
		while (_priority_states.size() > 1 && !_priority_states.front()->canBeParsed())
		{
			...
			catch (const std::exception &e)
			{
				...
				{
					_parsedRequest->set_parsing_validity_state(PARSE_FATAL_FAILURE);
		...
		if (_priority_states.size() == 0)
		{
			_parsedRequest->set_parsing_validity_state(PARSE_UNEXPECTED_END_PATTERN);
			break;
		}
		if (((unsigned int)this->_priority_states.front()->getResetRequestIndex()) >= this->_request.size())
		{
			_parsedRequest->set_parsing_validity_state(PARSE_NOTHING_MORE);
			break;
		}
		...
		try
		{
			_parsedRequest->set_parsing_validity_state((this->*GrammarParser::_builderDictionnary[id].second)(token, gp, id));
		}
		catch (const std::exception &e)
		{
	...
	} while (_parsedRequest->get_parsing_validity_state() != PARSE_NOT_ENOUGH_DATAS && _parsedRequest->get_parsing_validity_state() != PARSE_FATAL_FAILURE);

	// one_file_logger_raw(RESET);
	return _parsedRequest->get_parsing_validity_state();
}

ParsedRequest *GrammarParser::finishParse()
{
	e_parsing_states result = parse();
	if (result != PARSE_NOTHING_MORE)
	{
		ParsedRequest *parsedRequest = this->_parsedRequest->end_build(this->_request, result);
		clear();
...
e_parsing_states GrammarParser::consume_VAR(std::string token, GrammarParserBuilderMarker *gp, int id)
{
	...
	if (gv->getType() == ONLY_VALUE)
	{
		...
		this->_parsedRequest->add_key_parsedDatas("#" + gv->getName());
		this->_parsedRequest->set_value_buffer_parsedDatas(gp->getBufferPtr());
	}
...
ParsedRequest *GrammarParser::getParsedRequest()
{
