...
bool MasterServer::execKILL(std::string base, t_client_ParsedCmd &parsed_command)
{
	(void)base;
	...
	std::string notification = "Good bye !";
	if (message.size())
		notification = message.front();
	Client *victim = findClientWithNick(params.front());
	if (!victim)
	{
		pushToQueue(client->_fd, CodeBuilder::errorToString(ERR_NOSUCHNICK, this, client, &params.front()));
		return true;
	}
	pushToQueue(victim->_fd, ":" + getFullClientID(client) + " NOTICE : Kicked :" + notification + END_OF_COMMAND);

	_disconnectList.insert(victim->getFd());
