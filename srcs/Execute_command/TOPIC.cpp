...
bool MasterServer::execTOPIC(std::string base, t_client_ParsedCmd &parsed_command, std::vector<t_clientCmd> &respQueue)
{
	(void)base;
	(void)parsed_command;
	(void)respQueue;
	Client *client = parsed_command.first; // should not be null regarding how we got here
    if (HAS_TYPE(client->getMode(), ))
	...
	if (!current_chan)
	{
		pushToQueue(client->_fd, CodeBuilder::errorToString(ERR_NOSUCHCHANNEL, this, client, &channels.front()), respQueue);
		return true;
	}
